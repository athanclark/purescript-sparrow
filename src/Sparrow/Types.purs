...
module Sparrow.Types
  ( ClientReturn, ClientArgs, Client
  , Topic, topicToPath
  , SessionID, newSessionID
  , WithSessionID (..), WithTopic (..), InitResponse (..), WSHTTPResponse (..), WSIncoming (..), WSOutgoing (..)
  ) where
...
-- | Return value from a server, with a closed handler for sending @deltaIn@ values
...
-- | Arguments necessary for creating a sparrow topic connection
...
-- | Given a method for sending a @initIn@, receive a subscription
...
newSessionID :: Effect SessionID
newSessionID = SessionID <$> genUUID
